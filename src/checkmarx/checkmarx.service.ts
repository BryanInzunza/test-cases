...
export class CheckmarxService {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => TestCasesService)) // Usamos forwardRef aquí
  ...
  async create(createCheckmarxDto: CreateCheckmarxDto, file) {
    ...
    try {
      ...
      const aplicacion = await this.testCasesService.findOne(createCheckmarxDto.idu_aplicacion);
  ...
  async findOneByApplication(id: number) {
    ...
    const aplicacion = await this.testCasesService.findOne(id);
